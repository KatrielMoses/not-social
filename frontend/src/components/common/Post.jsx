...
const Post = ({ post }) => {
	...
	const { mutate: deletePost, isPending } = useMutation({
		mutationFn: async () => {
			try {
				...
				if (!res.ok) {
					throw new Error(data.error || "Post cannot be deleted")
				}
		...
		onMutate: async () => {
			...
			queryClient.setQueriesData({ queryKey: ['posts'] }, (oldData) => {
				...
				return oldData.map((p) => {
					if (p._id === post._id) {
						const userLikedPost = p.likes.includes(authUser._id);
						const updatedLikes = userLikedPost
	...
	const isLiked = post.likes && authUser?._id ? post.likes.includes(authUser._id) : false;
